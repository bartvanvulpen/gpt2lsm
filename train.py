...
def train():
    ...
    def update(engine, batch):
        ...
        # # ------ LSM LOSS ADDITION ------------
        M = LSMLoss(input_ids, lm_predicts, token_type_ids, lm_labels, tokenizer)
        lsm_loss, _, _ = M.loss()

        print("LSM Loss:", lsm_loss)
        #
        # set coefficient value (probably will be pushed into to the args)
        lsm_coef = 1.0

        loss = (lm_loss * args.lm_coef + mc_loss * args.mc_coef + lsm_coef * lsm_loss) / args.gradient_accumulation_steps
